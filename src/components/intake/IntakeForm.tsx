...
import type { ServiceType, IntakeFormData, BrandData, WebData, AppData, RevOpsData } from '@/lib/intake-schema'
import { intakeSchema, flattenErrors, defaultBrand, defaultWeb, defaultApp, defaultRevOps } from '@/lib/intake-schema'
...
const INITIAL_FIELDS: IntakeFormData = {
  ...
  services: [],
  situation: '', previousAttempts: '', decisionMaker: '',
  budget: '', timeline: '',
  anythingElse: '', honeypot: '',
  brand: defaultBrand, web: defaultWeb, app: defaultApp, revops: defaultRevOps,
}
  ...
  switch (action.type) {
    ...
    case 'TOGGLE_SERVICE': {
      ...
      const cleared: Partial<IntakeFormData> = removing
        ? {
          brand: action.service === 'brand' ? defaultBrand : state.fields.brand,
          web: action.service === 'web' ? defaultWeb : state.fields.web,
          app: action.service === 'app' ? defaultApp : state.fields.app,
          revops: action.service === 'revops' ? defaultRevOps : state.fields.revops,
        }
        : {}
      return {
        ...state,
...
export default function IntakeForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const allPaths = [
      'name', 'email', 'services', 'situation', 'budget', 'timeline',
      'brand.businessDescription', 'brand.customerDescription', 'brand.competitors', 'brand.personality',
      'web.primaryJob', 'web.brokenAndCost',
      'app.coreFunction', 'app.userType', 'app.specStatus',
      'revops.brokenProcess', 'revops.problemType', 'revops.successAt90Days',
    ]
    ...
    if (!result.success) {
      ...
      if (firstErrPath) {
        const el = document.querySelector(`[aria-invalid='true'], [data-error-field='${firstErrPath}']`)
